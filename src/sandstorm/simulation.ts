...
export class SandstormSimulation extends LimitedFrameRateCanvas {
    readonly #scale: number;
    readonly #wind: number;
    readonly #turbulence: number;
    ...
    draw(): void {
        ...
        for (const grain of this.#grains) {
            const px = grain.x * this.width;
            const py = grain.y * this.height;
            ...
            // Draw as short streak along wind direction
    ...
    tick(): void {
        ...
        for (let index = 0; index < this.#grains.length; index++) {
            ...
            grain.x += (grain.vx * dt) / this.width;
            grain.y += (grain.vy * dt) / this.height;
