...
export class LanternSimulation extends LimitedFrameRateCanvas {
    readonly #scale: number;
    readonly #speed: number;
    readonly #size: number;
    ...
    draw (): void {
        ...
        // Sort by size so smaller (further) lanterns render behind larger (closer) ones.
        const sorted = [...this.#lanterns].sort((a, b) => a.size - b.size);

        for (const lantern of sorted) {
            const px = lantern.x * this.width;
            const py = lantern.y * this.height;
            ...
            // Lantern body (rounded trapezoid shape)
            ctx.save();
            ctx.translate(px, py);
    ...
    tick (): void {
        ...
        for (let i = 0; i < this.#lanterns.length; i++) {
            ...
            // Rise upward
            lantern.y -= (lantern.vy * this.#speed * dt) / (this.height * 1.5);

            // Sway side to side
            const sway = Math.sin(this.#time * lantern.swaySpeed + lantern.swayPhase) * lantern.swayAmplitude;
            lantern.x += sway / (this.width * 8);

            // Recycle when past the top
            if (lantern.y < -0.15) {
