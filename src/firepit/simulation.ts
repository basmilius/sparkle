...
export class FirepitSimulation extends LimitedFrameRateCanvas {
    readonly #scale: number;
    readonly #flameWidth: number;
    readonly #flameHeight: number;
    readonly #intensity: number;
    ...
    #drawFlames(ctx: CanvasRenderingContext2D): void {
        ...
        for (let layerIndex = 0; layerIndex < this.#flameLayers.length; layerIndex++) {
            ...
            const wobbleX = Math.sin(this.#time * layer.speed + layer.phase) * layer.amplitude * this.width;
            const flameW = layer.width * this.width * this.#scale * 0.5;
            const flameH = layer.height * this.height * this.#scale;
        ...
        const glowGradient = ctx.createRadialGradient(
            centerX, baseY, 0,
            centerX, baseY, this.width * 0.35 * this.#scale
        );
    ...
    #drawEmbers(ctx: CanvasRenderingContext2D): void {
        ...
        for (const ember of this.#embers) {
            const px = ember.x * this.width;
            const py = ember.y * this.height;
